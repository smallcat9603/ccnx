                   ...
                   struct nameprefix_entry *npe)
{
    ...
    //huyao 130118 随机概率选择outgoing face
    //huyao 130206 FIB中没有此interest entry时，propagate到除incoming face外的所有faces
    if(outbound == NULL)
    {
                         ...
                         unsigned char *wire_msg, size_t wire_size)
{
    ...
    if (res == HT_OLD_ENTRY) {
        if (tailsize != e->extsize ||
            ...
            //huyao ccnx0.7.0此处有更新
            //huyao 130120
            process_content(h, msg, size, &obj, face, 5);
        }
        else {
...
//huyao 121219
void
process_content(struct ccnd_handle *h,
				unsigned char *msg,
				size_t size,
				...
				int NumOfFaces)
{
	...
	{
    	npe->forward_to->n++;
    	npe->forward_to->buf[1] = face->faceid;
	}
    ...
    npe->forward_to->n = NumOfFaces;


	boolean empty(){
		return (head%N == tail);
	}
	npe->forward_to->buf[faceindex++] = face->faceid;
	faceindex = faceindex%NumOfFaces;
	...
	//huyao 121223 参照ccnd_req_prefix_or_self_reg()和ccnd_req_unreg()
	///*
	struct ccn_forwarding_entry *forwarding_entry = NULL;
	const unsigned char *req;
	size_t req_size;
	...
	//process_incoming_content()已有函数ccn_parse_ContentObject()
	res = ccn_content_get_value(msg, size, &obj, &req, &req_size);
	forwarding_entry = ccn_forwarding_entry_parse(req, req_size);

	//以上是为了解出ccn_forwarding_entry *forwarding_entry,以下仿ccndc_add()
	forwarding_entry->faceid = face->faceid;
	//ccndc_do_prefix_action()作用不清楚，只是操作forwarding_entry？
	forwarding_entry->action = "prefixreg";

	//以下仿ccnd_req_prefix_or_self_reg（）
	if (forwarding_entry->lifetime < 0)
	forwarding_entry->lifetime = 2000000000;
	else if (forwarding_entry->lifetime > 3600 &&
	forwarding_entry->lifetime < (1 << 30))
	...
	//fifo
	npe->forward_to->limit = NumOfFaces;
	npe->forward_to->n = NumOfFaces;
	int i;
	for(i=1; i<NumOfFaces; i++)
	{
		npe->forward_to->buf[i] = npe->forward_to->buf[i-1];
	}
	npe->forward_to->buf[0] = face->faceid;

	//还是仿ccnd_req_prefix_or_self_reg（）
	forwarding_entry->flags = res;
	forwarding_entry->action = NULL;
	forwarding_entry->ccnd_id = h->ccnd_id;
	forwarding_entry->ccnd_id_size = sizeof(h->ccnd_id);

	struct ccn_charbuf *reply_body = ccn_charbuf_create();
	res = ccnb_append_forwarding_entry(reply_body, forwarding_entry);
	...
	//ccndc_do_prefix_action(ccndc, "prefixreg", forwarding_entry);
